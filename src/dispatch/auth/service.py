...
def create(*, db_session, user_in: UserRegister) -> DispatchUser:
    ...
    user = DispatchUser(**user_in.dict(exclude={
        "password", "en_code","import_sample_data","planner_code"
        }), password=password)
    db_session.add(user)
    db_session.commit()
    return user
...
def update(*, db_session, user: DispatchUser, user_in: UserUpdate, token:str, current_user_id:int) -> DispatchUser:
    """Updates a user."""
...
def get_or_set_user_redis(user_email=None, user_id=None, reload=False) -> DispatchUser:
    if user_email is not None:
        user_redis_key = REDIS_KEY_TEMPLATE_USER.format(user_email) 
    elif user_id is not None:
        user_redis_key = REDIS_KEY_TEMPLATE_USER.format(f"id:{user_email}")
    else:
    ...
    if user_json:
        ...
        redis_conn.set(user_redis_key, json.dumps(user_.dict()), ex=DISPATCH_JWT_EXP)
        session_local.close()
        return user_
