...
from dispatch.plugins.kandbox_planner.env.env_enums import JobPlanningStatus, JobType, LocationType
    ...
    for _, loc in gps_df.iterrows():
        ...
        gps_dict = {
            ...
            "geo_json": {
                ...
                "formatted_address_text": address,
            }
        }

        return_data[real_job_id] = {"worker_id": worker_id, "job_addr": gps_dict, 'row': loc}
...
def generate_one_day_orders(current_day, worker_list, nbr_jobs, job_start_index, auto_planning_flag):
    ...
    for real_job_id, job_item in group_job_data.items():
        ...
        i += 1
        row = job_item['row']
        job_code = "{}-{}-{}".format(datetime.strftime(current_day,
                                     "%m%d"), loc_job_index, real_job_id)
        ...
        start_location = _location_list[-1]
        end_localtion = (gps["geo_longitude"], gps["geo_latitude"])
        m = travel_router.get_travel_minutes_2locations(start_location, end_localtion)
        scheduled_start += m

        _location_list.append((gps["geo_longitude"], gps["geo_latitude"]))
...
def dispatch_jobs_batch_optimizer(opts):
    ...
    current_minutes = (GENERATOR_START_DATE - datetime.strptime(DATA_START_DAY,
                       KANDBOX_DATE_FORMAT)).days * 1440 + 1
...
def generate_all(opts):
    ...
    for day_i in range(999):
        ...
        if current_day >= GENERATOR_END_DATE:
            break
        job_list = generate_one_day_orders(current_day=current_day,
                                           worker_list=worker_list,
                                           nbr_jobs=opts["nbr_jobs"],
