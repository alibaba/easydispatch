...
from dispatch.config import APPOINTMENT_DEBUG_LIST, JOB_RECOMMENDATION_INTERVAL, VROOM_SOLVER_NB_THREADS,JOBINSLOT_REQUESTED_WORKER
import copy
...
import logging
log = logging.getLogger("single_realtime_rt31")


class SingleVroomMultiSlotsAgentPlugin(KandboxAgentPlugin):
    """
    RT3 is vroom solver based realtime agent for single job dispatch.

    """
    title = "Kandbox Plugin - Agent - RT31"
    slug = "single_realtime_rt31"
    author = "Kandbox"
    ...
    config_form_spec = {
        "type": "object",
        "properties": {
            "try_requested_worker": {
                "type": "string",
                "default": "0",
                "code": "try_requested_worker",
            }, 
            "nbr_of_actions": {
                "type": "number",
                "default": 5,
    ...
    def dispatch2slot(
        ...
        try_closest_workers = str(self.config["try_closest_workers"]) == "1"

        try_requested_worker =  str(self.config.get("try_requested_worker", "0"))  == "1"
        ...
        for si, s in enumerate(slots): 
            ...
            job_code_set.add(s.slot_code)
            worker_start_loc_idx[s.worker_code] = len(jobs_locs) - 1
            # jobs_locs.append(
            #         OptiJobLoc(
            #             geo_longitude = s.start_longitude,
            #             geo_latitude = s.start_latitude,
        ...
        for si, _slot in enumerate(slots): 
            log.info(f"realtime_job_info: adding slot = { _slot.slot_code }, assigned_jobs = {[j.code for j in _slot.assigned_jobs]}")
            for j in  _slot.assigned_jobs:
                if j.code in job_code_set:
                    log.error(f"duplicated job code detected: {j.code} at slot {_slot.slot_code}")
                    continue
                jobs_locs.append(
                    OptiJobLoc(
        ...
        if target_index_inside == -1:
            ...
            fwi = None
            if try_requested_worker and (JOBINSLOT_REQUESTED_WORKER in curr_job.flex_form_data):
                if curr_job.flex_form_data[JOBINSLOT_REQUESTED_WORKER] in worker_start_loc_idx:
                    fwi = worker_start_loc_idx[curr_job.flex_form_data[JOBINSLOT_REQUESTED_WORKER]]
                else:
                    log.warning(f"requested worker {curr_job.flex_form_data[JOBINSLOT_REQUESTED_WORKER]} is not included for job {curr_job.code}.")
            if j.code in job_code_set:
                log.error(f"current_target_is_duplicated job code detected: {j.code} at slot {_slot.slot_code}")
            ...
            jobs_locs.append(
                        OptiJobLoc(
                            ...
                            c_volume = 1,
                            c_time_window = [],
                            fixed_worker_index = fwi,
                            flex_form_data= curr_job.flex_form_data,
        ...
        log.info(f"realtime_job_info: slot = {[j.slot_code for j in slots]}, all_job_locs = {[j.code for j in jobs_locs]}")
        ...
        if solution:
            ...
            log.warning("RT31_Realtime_Dispatching_Failed.")
        return []
