...
def print_solution(agent, manager, routing, solution):
    ...
    for vehicle_id in range(len(agent.env.workers)):
...
class OrtoolsCVRPPlanner(KandboxBatchOptimizerPlugin):
    ...
    def dispatch_jobs(self, env):
        ...
        manager = pywrapcp.RoutingIndexManager(len(self.job_locations), len(self.env.workers), 0)
        ...
        while current_date < GENERATOR_END_DATE:
            ...
            day_seq = int(self.env.env_encode_from_datetime_to_minutes(current_date) / 1440)
            ...
            for job in job_list:
                ...
                if internal_result_info.status_code != ActionScoringResultType.OK:
                    print(f"{one_job_action_dict.job_code}: Failed to commit change, error: {str(internal_result_info)} ")
                ...
                job_to_update = JobPlanningInfoUpdate (
                    ...
                    scheduled_start_datetime= self.env.env_decode_from_minutes_to_datetime(
                        job["scheduled_start_minutes"]),
                    scheduled_duration_minutes = job["scheduled_duration_minutes"],
                    scheduled_primary_worker_code  =  job["scheduled_primary_worker_id"] ,
                )
                job_service.update_planning_info(db_session = self.env.kp_data_adapter.db_session, job_in = job_to_update)
