...
import math
from .single_job2slot_n2s_env import Step_State
...
def get_initial_insert_mask(step_state:Step_State) -> torch.Tensor:
    ...
    worker_free_map = step_state.worker_selected_loc_length < step_state.max_job_in_worker_size
    worker_job_free_map = torch.ones(step_state.batch_pomo_size, step_state.worker_job_size_plus1).bool()
    worker_job_free_map = worker_job_free_map.scatter(1, curr_idx, worker_free_map)

    
    # if not worker_free_map.all():
    _step_i = 0
    while _step_i < step_state.max_job_in_worker_size and (next_idx != step_state.worker_job_size).any().item():
        _step_i +=1
        curr_idx = next_idx
    ...
    #     worker_job_free_map.view(step_state.batch_pomo_size, step_state.worker_job_size_plus1,1).repeat(1,1, step_state.worker_job_size_plus1), 
    #     worker_job_free_map.view(step_state.batch_pomo_size,1, step_state.worker_job_size_plus1).repeat(1, step_state.worker_job_size_plus1,1),
    # ) 
    
    # worker_minus_ = step_state.solution_worker_idx.clone()
    # worker_minus_[(worker_minus_ < 0)] = worker_minus_[(worker_minus_ < 0)] - 1 
    ...
    allowed_nodes_map = torch.logical_and(
        allowed_nodes,
        worker_job_free_map[:, :step_state.worker_job_size]
        )

    return ~allowed_nodes_map # here returning true means unavailable/*masked*



def get_swap_insert_mask(step_state:Step_State, selected_pick, allow_changing_worker_flag: bool = False) -> torch.Tensor: # , selected_drop
    # When we swap, we can only swap the selected job to same work that it was assigned to, as selected_worker
    # No cross worker swap.
    if allow_changing_worker_flag:
        selected_worker_map = torch.ones(step_state.batch_pomo_size, step_state.worker_job_size).bool()
    else:
        selected_worker = step_state.solution_worker_idx[
            torch.arange(step_state.batch_pomo_size),
    ...
    # TODO, maybe allowing same position back can help? 2023-11-03 15:03:34
    selected_worker_map[torch.arange(step_state.batch_pomo_size),selected_pick]=False
    return ~selected_worker_map
    ...
    # selected_worker_mask[torch.arange(step_state.batch_pomo_size),selected_pick,:]=False
    # selected_worker_mask[torch.arange(step_state.batch_pomo_size),:,selected_pick]=False
    # selected_worker_mask[torch.arange(step_state.batch_pomo_size),selected_drop,:]=False
    # selected_worker_mask[torch.arange(step_state.batch_pomo_size),:,selected_drop]=False
...
def get_removal_node_mask(step_state:Step_State) -> torch.Tensor:
    ...
    # only when there are more than 2 jobs assigned to the worker, there is a chance to swap.
    multi_job_worker_map = step_state.worker_selected_loc_length > 2

    # Then overlay the slot mask if required.
    if step_state.enabled_slot_map is not None:
        allowed_worker_map = torch.logical_and( 
            multi_job_worker_map, 
            step_state.enabled_slot_map,
        ) 
    else:
        allowed_worker_map = multi_job_worker_map


    curr_idx = torch.arange(step_state.worker_size).view(1,step_state.worker_size).repeat(step_state.batch_pomo_size,1)
    next_idx = step_state.solution[step_state.BATCH_POMO_IDX_2D, curr_idx] 
    job_worker_map = torch.ones(step_state.batch_pomo_size, step_state.worker_job_size_plus1).bool()
    job_worker_map = job_worker_map.scatter(1, curr_idx, allowed_worker_map)
    # worker_start_seq = curr_idx.clone()*100
    # step_state.solution[0:2,5]=14
    # curr_idx = w_initials
    # next_idx = step_state.solution[step_state.BATCH_POMO_IDX_2D, curr_idx]
    _step_i = 0
    # Only first pick job can be possible to block
    while _step_i < step_state.max_job_in_worker_size:
        _step_i +=1
        curr_idx = next_idx
        step_state.solution_job_curr_seq = step_state.solution_job_curr_seq.scatter(1, curr_idx, _step_i)
        job_worker_map = job_worker_map.scatter(1, curr_idx, allowed_worker_map)
...
class NodePairReinsertionDecoder(nn.Module):  # (14) (15)
    def __init__(self, n_heads: int, input_dim: int) -> None:
        ...
        # 2 heads are from point-wise, first_before/after, no delivery 2nd
        # 4 heads are from worker_mean_mlp
        self.agg = MLP ((2+NBR_WORKER_MEAN_HEAD) * n_heads, 32, 32, 1, p_dropout=DROPOUT_PERCENT)
    ...
    def forward(
        self,step_state, encoded_nodes, pos_pickup 
    ) -> torch.Tensor:
        # h_hat: torch.Tensor,
        ...
        shp_p = (batch_pomo_size, worker_job_size, self.n_heads)
        # shp_d = (batch_pomo_size, 1, worker_job_size, self.n_heads)
        shp_mean_1d = (batch_pomo_size, worker_job_size, self.n_heads*NBR_WORKER_MEAN_HEAD)
        # shp_mean_2d = (batch_pomo_size, worker_job_size, worker_job_size, self.n_heads*NBR_WORKER_MEAN_HEAD)

        arange = torch.arange(batch_pomo_size, device=h_hat.device)
        h_pickup = h_hat[arange, pos_pickup].unsqueeze(1)  # (batch_pomo_size, 1, input_dim)
        ...
        # TODO, put this mean and diff to pre_forward.!!!2023-11-03 11:23:42
        ...
        encoded_nodes_diff_pomo_repeated = encoded_nodes_diff.view(
        ...
        job_worker_head = self.worker_mean_mlp(
            job_worker_mean
        ).view(shp_mean_1d)  # (batch_size, graph_size+1, n_heads)
        ...
        return compatibility.view(batch_pomo_size, worker_job_size)  # (batch_size, graph_size+1)
...
class NodePairRemovalDecoder(nn.Module):  # (12) (13)
    ...
    def forward(
        ...
        compatibility_agg = self.agg(compatibility[
            :, :, worker_size : worker_size + job_size 
            ].permute(1, 2, 0)
        ) # .squeeze()  # (batch_size, graph_size/2)

        return compatibility_agg
