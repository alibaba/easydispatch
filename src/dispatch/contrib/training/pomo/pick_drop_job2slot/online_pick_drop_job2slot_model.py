# 2022-06-29 22:18:19
# This is V2 of online, which means we know only the job nodes given so far, we do not know all job locations.
# In this V2, I tile all (eg. 10) jobs in one worker vector and feed 20 numbers into the transformer.
...
class OnlinePickDropJob2SlotModel(nn.Module):
    ...
    def forward(self, step_state):
        batch_size, pomo_size, wrkr_size,max_job_in_worker_size = step_state.all_worker_locs_to_dispatch.size()
    ...
    def _get_online_worker_n_job_encoding(self, step_state):
        ...
        batch_size, pomo_size, wrkr_size, max_job_in_worker_size = step_state.all_worker_locs_to_dispatch.size()
        worker_size = wrkr_size - 1



        all_worker_locs = step_state.all_worker_locs_to_dispatch.view(
            batch_size*pomo_size,
            wrkr_size,
            max_job_in_worker_size
        )
        embedded_paths_all = self.encoder(all_worker_locs)
        embedding_dim = self.model_params["embedding_dim"]
        embedded_worker_nodes = embedded_paths_all[:,0:worker_size,:].view(
            batch_size,pomo_size,
            worker_size,
            embedding_dim
        )

        embedded_job_nodes = embedded_paths_all[
        ...
        return embedded_worker_nodes, embedded_job_nodes 
...
class Job2Slot_Encoder(nn.Module):
    def __init__(self, **model_params):
        ...
        self.embedding_node = nn.Linear(
            #10*2, 
            self.model_params['max_job_in_worker_size'] * self.model_params['loc_dim'],
            embedding_dim)
        self.layers = nn.ModuleList([EncoderLayer(**model_params) for _ in range(encoder_layer_num)])

    def forward(self, loc_xy):
        # print(f"\tGPU - Encoder: loc_xy {loc_xy.device}, size: {loc_xy.size()} - self.embedding_node ({self.embedding_node.weight.device }):  " )
...
class Job2Slot_Decoder(nn.Module):
    ...
    def forward(self, encoded_current_jobs, encoded_workers, step_state):
        ...
        #######################################################
        encoded_workers_3d = encoded_workers.reshape(batch_size, pomo_size*worker_size, embedding_size)
