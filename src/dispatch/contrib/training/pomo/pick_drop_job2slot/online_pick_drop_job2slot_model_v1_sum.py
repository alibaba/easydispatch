# 2022-06-29 22:18:19
# This was the version v1 online, taking all points average, without start and end difference
# Snapshop taken before converting into a tiled worker transformer.
...
class OnlinePickDropJob2SlotModel(nn.Module):
    ...
    def forward(self, step_state):
        batch_size = step_state.reset_state.BATCH_IDX.size(0)
        pomo_size = step_state.reset_state.BATCH_IDX.size(1)
        ...
        if self.training or self.model_params['eval_type'] == 'softmax':
            while True:  # to fix pytorch.multinomial bug on selecting 0 probability elements
                ...
                # shape: (batch, pomo)
                prob = probs[step_state.reset_state.BATCH_IDX, step_state.reset_state.POMO_IDX, selected].reshape(batch_size, pomo_size)
                # shape: (batch, pomo)
                if (prob != 0).all():
                    break
    ...
    def _get_online_worker_n_job_encoding(self, step_state):
        ...
        # current_job_index = step_state.reset_state.job_loc_idx[
        ...
        drop_idx = step_state.current_job_idx + step_state.reset_state.job_size

        #  = torch.zeros(size=( batch_size, pomo_size, 1),dtype=torch.int64) + current_job_idx
        pick_gathering_index = step_state.reset_state.job_loc_idx[
            :,:,pick_idx:pick_idx+1
            ].view(
            batch_size*pomo_size,
            1, 
        ...
        drop_gathering_index = step_state.reset_state.job_loc_idx[
            :,:,drop_idx:drop_idx+1
            ].view(
            batch_size*pomo_size,
            1, 
        ...
        all_jobs_gather_index = torch.cat((
            worker_job_index, 
            pick_gathering_index,
            drop_gathering_index,
            ), dim=1).view(
            batch_size,
            pomo_size * (worker_size*max_job_in_worker_size + 2), 
        )[:,:,None].repeat(
            1,1,2 
        )

        picked_addr = step_state.reset_state.loc_xy.gather(dim=1, index=all_jobs_gather_index).view(
            batch_size*pomo_size,
            worker_size*max_job_in_worker_size + 2,
            2 # embedding_dim
        )
        ...
        embedded_job_nodes = embedded_nodes_all[
            :,worker_size*max_job_in_worker_size:worker_size*max_job_in_worker_size+2,:
        ].view(
            batch_size,pomo_size,
            # 1, 1,
            embedding_dim * 2
        ...
        merged_nodes = self.pickdrop2one_linear_1(embedded_job_nodes)



        return embedded_worker_nodes.mean(dim = 3), merged_nodes 
    ...
    def _get_job_encoding(self, step_state, batch_size, pomo_size):
        ...
        #  = torch.zeros(size=( batch_size, pomo_size, 1),dtype=torch.int64) + current_job_idx
        gathering_index = step_state.reset_state.job_loc_idx[
            :,:,pick_idx:pick_idx+1
