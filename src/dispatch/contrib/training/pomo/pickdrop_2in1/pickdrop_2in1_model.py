...
class PickDrop2in1Model(nn.Module):
    ...
    def __init__(self, model_params):
        ...
        self.pickdrop2one_linear_1 = nn.Linear(self.embedding_dim*2, self.embedding_dim, bias=False)
    ...
    def forward(self, step_state):
        if step_state.job2slot_tsp_step == 0:
            return self.forward_job2slot(step_state)
        else:
            return self.forward_tsp(step_state)


    def forward_job2slot(self, step_state): 
        encoded_workers = self._get_worker_encoding(step_state)
        encoded_current_jobs = self._get_job_encoding(step_state, self.batch_size, self.pomo_size)
        ...
        if self.training and self.model_params['eval_type'] == 'softmax': # changed from or to add 2023-03-25 07:57:55
            ...
            while True:  # to fix pytorch.multinomial bug on selecting 0 probability elements
                with torch.no_grad():
                    selected = probs.reshape(self.batch_size * self.pomo_size, -1).multinomial(1) \
                        .squeeze(dim=1).reshape(self.batch_size, self.pomo_size)
                # shape: (batch, pomo)
                prob = probs[self.BATCH_IDX, self.POMO_IDX, selected[:,:,None]].reshape(self.batch_size, self.pomo_size)
                # shape: (batch, pomo)
                if (prob != 0).all():
                    break
            ...
            # shape: (batch, pomo)
            prob = torch.ones(selected.size())  # value not needed. Can be anything.

        return selected, prob, probs



    def forward_tsp(self, step_state):

        gathering_index = step_state.tsp_current_loc.expand(self.batch_size, self.tsp_pomo_size, self.embedding_dim) # [:,:, None]
        # shape: (batch, pomo, embedding)
        encoded_last_node = self.encoded_nodes.gather(dim=1, index=gathering_index)
        # shape: (batch, pomo, embedding)

        # shape: (batch, pomo, embedding)
        new_ninf_mask = step_state.tsp_next_job_mask[:,:,0:self.problem_size] # torch.log, not need when using -inf as mask
        probs = self.tsp_decoder(encoded_last_node, ninf_mask=new_ninf_mask)
        # shape: (batch, pomo, problem)

        if self.training or self.model_params['eval_type'] == 'softmax':
            selected = probs.reshape(self.batch_size * self.tsp_pomo_size, -1).multinomial(1) \
                .squeeze(dim=1).reshape(self.batch_size, self.tsp_pomo_size)
            # shape: (batch, pomo)

            prob = probs[torch.arange(self.batch_size)[:,None,None], torch.arange(self.tsp_pomo_size)[None,:,None], selected[:,:, None]
            ].reshape(self.batch_size, self.tsp_pomo_size)
            # shape: (batch, pomo)
        ...
        # return selected.reshape(self.batch_size, self.pomo_size, self.worker_size), \
        #     prob.reshape(self.batch_size, self.pomo_size, self.worker_size), \
        #     probs.view(self.batch_size, self.pomo_size, self.worker_size, self.problem_size)
        return selected, prob, probs
    ...
    def _get_job_encoding(self, step_state, batch_size, pomo_size):
        ...
        #     torch.cat((pick_nodes, drop_nodes), dim = 2))
