...
class PickDrop2in1Env:
    def __init__(self, env_params, model_params):
        ...
        # self.tsp_model.eval()
        # print(f"Loaded inner TSP model for PickDrop2in1Env from: {file_name}")
    ...
    def load_jobs(self, 
        ...
        self.step_state.job2slot_tsp_step = 0
    ...
    def set_state_to_length(self, worker_loc_length, worker_selected_loc, enable_ninf_mask = None):
        ...
        # shape: (batch, pomo, self.num_pairs+1) # Worker location is the last one
        # self.job2slot_current_job_i = torch.zeros(size=(self.batch_size, self.pomo_size, 1)) + self.worker_size 
        if enable_ninf_mask is not None:
            self.step_state.enable_ninf_mask = enable_ninf_mask

        with torch.no_grad():
            self.step_state.job2slot_current_job_i = 0 # self.worker_size  # 
            self.step_state.worker_selected_loc_length = worker_loc_length
            self.step_state.worker_selected_loc_idx = worker_selected_loc
            # 更新.step_state.ninf_mask
            if self.step_state.enable_ninf_mask:
                self.step_state.ninf_mask = torch.zeros(size=(self.batch_size, self.pomo_size, self.worker_size))
                ...
                self.step_state.ninf_mask = None
    ...
    def step(self, selected):
        ...
        # ninf_mask: torch.Tensor = None
        with torch.no_grad():
            if self.step_state.job2slot_tsp_step == 0:
                return self.step_job2slot(selected=selected)
            else:
                return self.step_tsp(selected=selected)
    ...
    def step_job2slot(self, selected):
        ...
        selected_worker_length = self.step_state.worker_selected_loc_length[
                self.BATCH_IDX, self.POMO_IDX, selected_3d
            ] .long()
        curr_job_i = self.step_state.job2slot_current_job_i 
        ...
        self.step_state.worker_selected_loc_idx[
            self.BATCH_IDX, self.POMO_IDX, selected_3d, selected_worker_length 
            ] = self.step_state.reset_state.job_loc_idx[:,:,curr_job_i,0:1]

        self.step_state.worker_selected_loc_idx[
            self.BATCH_IDX, self.POMO_IDX, selected_3d, selected_worker_length+1
            ] = self.step_state.reset_state.job_loc_idx[:,:,curr_job_i,1:2]
        ...
        # 2.1. Convert worker_selected_loc_idx to tiled.
        self._calc_tiled_worker_selected_loc_idx()
        self.step_state.job2slot_current_job_i += 1
        ...
        if done or self.env_params["solve_tsp_in_each_step"]:
            self.step_state.job2slot_tsp_step = 1
            self.step_state.tsp_current_loc = self.step_state.reset_state.worker_loc_idx.view(
                self.batch_size, self.tsp_pomo_size,1
            )
            self.step_state.tsp_selected_count = 1
            # shape: (batch, pomo, 1)
            self.step_state.tsp_selected_loc_idx = self.step_state.tsp_current_loc # [:,:,None]
            ...
            loc_gathering_index = self.pad_indexer[flat_length,:].view(
                self.batch_size, self.pomo_size*self.worker_size, self.max_job_in_worker_size)
            
            self.step_state.tsp_starting_loc_idx = self.step_state.worker_selected_loc_idx.view(
                self.batch_size, self.pomo_size*self.worker_size, self.max_job_in_worker_size
            )[
                self.BATCH_IDX, self.TSP_POMO_IDX, loc_gathering_index
            ]
            

            self.step_state.tsp_next_job_mask = self.pad_masker[flat_length,:].view(
                self.batch_size, self.pomo_size*self.worker_size, self.max_job_in_worker_size+1)

            self.step_state.tsp_next_job_index = self.next_job_indexer[flat_length,:].view(
                self.batch_size, self.pomo_size*self.worker_size, self.max_job_in_worker_size)

            self.step_state.tsp_time_cumsum_mask = self.time_cumsum_masker[flat_length,:].view(
                self.batch_size, self.pomo_size*self.worker_size, self.max_job_in_worker_size)

        # leave it to TSP
        return self.step_state, reward, False
    ...
    def _get_travel_distance_after_pomo_worker_tsp(self):
        ...
        tsp_next_job_mask = self.pad_masker[flat_length,:].view(
            self.batch_size, self.pomo_size*self.worker_size, self.max_job_in_worker_size+1)
        ...
        tsp_gathered_job_indexes = self.step_state.worker_selected_loc_idx.view(
            self.batch_size, self.pomo_size*self.worker_size, self.max_job_in_worker_size) # add the last X,Y (long/lat) index

        tsp_pomo_size = self.pomo_size*self.worker_size
        ...
        # self.tsp_model.pad_mask = tsp_next_job_mask
    ...
    def step_tsp(self, selected):
        # TODO: TSP inside. 2022-02-27 22:32:07
        self.step_state.tsp_current_loc = self.step_state.tsp_starting_loc_idx[
            self.step_state.reset_state.BATCH_IDX, 
            self.step_state.reset_state.TSP_POMO_IDX, 
            selected[:, :, None]
        ]
        self.step_state.tsp_selected_count += 1
        self.step_state.tsp_selected_loc_idx = torch.cat((
            self.step_state.tsp_selected_loc_idx, 
            self.step_state.tsp_current_loc
        ), dim=2)


        unblocked_job_idx = self.step_state.tsp_next_job_index[
            torch.arange(self.batch_size)[:,None,None],
            torch.arange(self.tsp_pomo_size)[None,:,None], 
            selected[:, :, None]
        ]
        # shape: (batch, pomo, 1)
        self.step_state.tsp_next_job_mask[self.BATCH_IDX, self.TSP_POMO_IDX, unblocked_job_idx] = 0 # 1
        self.step_state.tsp_next_job_mask[self.BATCH_IDX, self.TSP_POMO_IDX, selected[:, :, None]] = float("-inf")# 0

        done = (self.step_state.tsp_selected_count >= self.max_job_in_worker_size)

        if done and (not self.env_params["serving_only_n_no_reward"]):
            travel_distances = self.get_tsp_travel_distance_by_matrix()
            reward = 0-travel_distances
            # Now I save the re-arranged job sequence in each worker from TSP ENV.
            # self.step_state.worker_loc_idx_after_tsp = self.step_state.worker_selected_loc_idx[
            #     self.I4D_Batch, self.I4D_Pomo, self.I4D_Worker,
            #     self.tsp_env.selected_node_list.view(
    ...
    def get_tsp_travel_distance_by_matrix(self,):
        ...
        ordered_nodes = self.ordered_nodes = self.step_state.tsp_selected_loc_idx 
        ...
        head_seq = ordered_nodes[:,:,0] # .view( self.batch_size, self.pomo_size,self.worker_size,1 ) 
        tail_seq = ordered_nodes[
            self.BATCH_IDX, 
            self.TSP_POMO_IDX, 
            (self.step_state.worker_selected_loc_length.view(self.batch_size, self.pomo_size*self.worker_size)-1)[:,:,None]
        ].squeeze(-1)
        ...
        if self.step_state.tsp_time_cumsum_mask is not None and ADDR_DIM > 2:
