...
class Reset_State:
    ...
    allowed_maximum_minutes: int = ALLOWED_MAXIMUM_MINUTES
    allowed_minutes_importance: int = 0.1
    encoded_nodes_mean: torch.Tensor = None
    encoded_nodes_diff: torch.Tensor = None
...
def step_remove_nodes(
    ...
    # solution_pre_idx = solution.argsort()  # pre=[1,2,0]
    ...
    pre_drop = step_state.solution_pre_idx.gather(1, pair_drop)
    post_drop = solution.gather(1, pair_drop)

    step_state.solution_pre_idx.scatter_(1, post_drop, pre_drop)
    solution.scatter_(1, pre_drop, post_drop)  # second -> pair_drop
    solution.scatter_(1, pair_drop, step_state.worker_job_size)  # reset this job pointing to end


    # post_second = solution.gather(1, target_second)
    # solution.scatter_(1, target_second, pair_drop)  # second -> pair_drop
    # step_state.solution_pre_idx.scatter_(1, pair_drop, target_second)

    # solution.scatter_(1, pair_drop, post_second)
    # step_state.solution_pre_idx.scatter_(1, post_second, pair_drop)
...
def step_insert_nodes(
    ...
    pair_pick = action[:, 1].view(step_state.batch_pomo_size, 1)
    # pair_drop = pair_pick + step_state.job_size
    pair_drop = action[:, 2].view(step_state.batch_pomo_size, 1)

    target_first = action[:, 3].view(step_state.batch_pomo_size, 1)
    ...
    assert (
        (pair_pick != target_first).all()
        and (pair_drop != target_second).all()
    )
    ...
    assert (
        selected_worker_idx
        ==
        step_state.solution_worker_idx.gather(1, target_second)
        ).all().item()
    ...
    selected_worker_idx = step_state.solution_worker_idx.gather(1, target_second)
    step_state.solution_worker_idx.scatter_(1, pair_drop, selected_worker_idx)
    step_state.solution_worker_idx.scatter_(1, pair_pick, selected_worker_idx)

    # 下面这个不好使，是链式的反应
    # new_curr_seq = step_state.solution_job_next_seq.gather(1, target_second)
    # step_state.solution_job_curr_seq.scatter_(1, target_second, new_curr_seq)
    # step_state.solution_job_next_seq.scatter_(1, target_second, new_curr_seq+1)
    # step_state.solution_job_next_seq.scatter_(1, pair_drop, new_curr_seq+1)
    # [step_state.BATCH_POMO_IDX_2D, target_second] = 
...
class PickDropPomoN2SEnv:
    def __init__(self, env_params, model_params):
        ...
        # self.tsp_model.eval()
        # print(f"Loaded inner TSP model for PickDropPomoN2SEnv from: {file_name}")
    ...
    def reset_indexer(self):
        ...
        MAX_NBR_JOB_IN_WORKER = self.env_params['max_job_in_worker_size'] # 10
        # MAX_FLOAT = 1e25
    ...
    def load_jobs(self, 
        ...
        self.step_state.job_size = self.num_pairs = job_loc_idx.size(2)
        self.step_state.worker_job_size = self.step_state.job_size * 2 + self.step_state.worker_size
        ...
        self.step_state.reset_state.job_size = job_loc_idx.size(2)
        ...
        self.TSP_POMO_IDX = torch.arange(self.tsp_pomo_size)[
            None, :, None]

        self.step_state.reset_state.BATCH_IDX = self.BATCH_IDX
        self.step_state.reset_state.POMO_IDX = self.POMO_IDX
        self.step_state.reset_state.TSP_POMO_IDX = self.TSP_POMO_IDX
        ...
        # _solu = torch.arange(self.step_state.worker_job_size_plus1)
        # _solu = self.step_state.worker_job_size # [self.step_state.worker_size:self.step_state.worker_job_size_plus1]
        # self.step_state.solution = _solu.view(
        #     1,self.step_state.worker_job_size+1
        # ).repeat(
        #     self.step_state.batch_pomo_size, 1)
        ...
        # self.step_state.solution = torch.cat([
        #     torch.zeros((self.step_state.batch_pomo_size, self.step_state.worker_size), dtype=torch.long) + self.step_state.worker_job_size,
        #     (torch.arange(self.step_state.job_size) + self.step_state.worker_size + self.step_state.job_size).view(1, self.step_state.job_size).repeat(self.step_state.batch_pomo_size, 1) ,
        #     torch.zeros((self.step_state.batch_pomo_size, self.step_state.job_size+1), dtype=torch.long) + self.step_state.worker_job_size,
        # ],  dim=-1)
        # torch.cat([
        #     worker_loc_idx, 
        #     job_loc_idx[:,:,:,0], 
        ...
        #     (0-torch.ones(self.step_state.batch_pomo_size, self.step_state.reset_state.job_size*2 + 1)).long(),  
        #     ],
        #     dim=-1)
        self.step_state.solution_worker_idx = torch.cat([
            worker_loc_idx.view(self.step_state.batch_pomo_size, self.step_state.reset_state.worker_size), 
            (-1000 - torch.arange(self.step_state.reset_state.job_size*2 + 1)).long().view(
                1, self.step_state.reset_state.job_size*2 + 1).repeat( self.step_state.batch_pomo_size,1 ),  
            ],
            dim=-1)

        self.step_state.solution_job_curr_seq = torch.zeros(self.step_state.solution_worker_idx.size(), dtype=torch.long)
        # self.step_state.solution_job_next_seq = torch.cat([
        #     torch.ones((self.step_state.batch_pomo_size, self.worker_size), dtype=torch.long),
        #     torch.zeros((self.step_state.batch_pomo_size, self.num_pairs*2 + 1), dtype=torch.long),
        #     ],
        #     dim=-1)
        self.step_state.solution_pre_idx = torch.cat([
            worker_loc_idx.view(self.step_state.batch_pomo_size, self.step_state.reset_state.worker_size), 
            (self.step_state.worker_size + torch.arange(self.step_state.reset_state.job_size*2 + 1)).long().view(
                1, self.step_state.reset_state.job_size*2 + 1).repeat( self.step_state.batch_pomo_size,1 ),  
            ],
            dim=-1)

        # self.step_state.prev_best_solution = torch.zeros((2, self.step_state.batch_pomo_size, self.step_state.worker_job_size_plus1), dtype=torch.long) + self.step_state.worker_job_size
        # self.step_state.prev_best_obj = torch.ones((2, self.step_state.batch_pomo_size,), dtype=torch.int64) + 998
        # self.step_state.prev_best_step = torch.ones((2, self.step_state.batch_pomo_size,), dtype=torch.int64)
    ...
    def reset(self):
        ...
        # self.job2slot_current_job_i = torch.zeros(size=(self.batch_size, self.pomo_size, 1)) + self.worker_size 
        with torch.no_grad():
            ...
            # shape: (batch, pomo)
            done = self.finished = (self.step_state.job2slot_current_job_i >= self.num_pairs)
            reward = None
    ...
    def step_job2slot(self, selected):
        ...
        reward = 0
        done = self.step_state.job2slot_current_job_i >= self.num_pairs # self.worker_size + 
        if done:
