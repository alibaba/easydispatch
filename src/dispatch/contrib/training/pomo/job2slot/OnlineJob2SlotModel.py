...
class OnlineJob2SlotModel(nn.Module):
    ...
    def forward(self, step_state):
        ...
        encoded_workers, encoded_current_jobs = self._get_online_worker_n_job_encoding(step_state)
    ...
    def _get_worker_encoding(self, step_state):
        ...
        job_gather_index = step_state.worker_selected_loc_idx[:,:,:,:,None].repeat(
            1,1,1,1, 2 # embedding_dim
        ) # shape: [ batch, (pomo, worker, max_jobs), embedding ]

        # job_gather_index = step_state.worker_selected_loc_idx.view(
        #     batch_size,  pomo_size*worker_size*max_job_in_worker_size, 1
        # )
        ...
        picked_addr = self.reset_state.loc_xy.gather(dim=1, index=job_gather_index_3d).view(
            batch_size*pomo_size,
            worker_size*max_job_in_worker_size,
            2 # embedding_dim
        )
        picked_nodes = self.encoder(picked_addr).view(
            batch_size,pomo_size,
            worker_size, max_job_in_worker_size,
            embedding_dim
        )
    ...
    def _get_job_encoding(self, step_state, batch_size, pomo_size):
        ...
        gathering_index = self.reset_state.job_loc_idx[
            :,:,step_state.current_job_idx:step_state.current_job_idx+1
            ].repeat(1,1,2) # 2 is addr X.Y size
        # gathering_index = node_index_to_pick.expand(batch_size, pomo_size, embedding_dim)
        # shape: (batch, pomo, embedding)
        picked_addr = self.reset_state.loc_xy.gather(dim=1, index=gathering_index).view(
            batch_size,
            pomo_size,
            2 # embedding_dim
        )
        picked_nodes = self.encoder(picked_addr)
    ...
    def _get_online_worker_n_job_encoding(self, step_state):
        embedding_dim = self.model_params["embedding_dim"]
        batch_size, pomo_size, worker_size,max_job_in_worker_size = step_state.worker_selected_loc_idx.size()

        # For safety, convinience, just all job slots.
        # max_job_in_worker_size = step_state.worker_selected_loc_length.max() # dim=2
        # job_gather_index = step_state.worker_selected_loc_idx.view(
        #     batch_size,  pomo_size*worker_size*max_job_in_worker_size, 1
        # )

        worker_job_index = step_state.worker_selected_loc_idx[:,:,:,0:max_job_in_worker_size].view(
            batch_size*pomo_size,
            worker_size*max_job_in_worker_size, 
        )
        current_job_index = self.reset_state.job_loc_idx[
            :,:,step_state.current_job_idx:step_state.current_job_idx+1
        ].view(
            batch_size*pomo_size,
            1, 
        )
        all_jobs_gather_index = torch.cat((worker_job_index, current_job_index), dim=1).view(
            batch_size,
            pomo_size * (worker_size*max_job_in_worker_size + 1), 
        )[:,:,None].repeat(
            1,1,2 
        )

        picked_addr = self.reset_state.loc_xy.gather(dim=1, index=all_jobs_gather_index).view(
            batch_size*pomo_size,
            worker_size*max_job_in_worker_size + 1,
            2 # embedding_dim
        )

        embedded_nodes_all = self.encoder(picked_addr)
        embedded_worker_nodes = embedded_nodes_all[:,0:worker_size*max_job_in_worker_size,:].view(
            batch_size,pomo_size,
            worker_size, max_job_in_worker_size,
            embedding_dim
        )
        embedded_job_nodes = embedded_nodes_all[
            :,worker_size*max_job_in_worker_size:worker_size*max_job_in_worker_size+1,:
        ].view(
            batch_size,pomo_size,
            # 1, 1,
            embedding_dim
        ...
        return embedded_worker_nodes.mean(dim = 3), embedded_job_nodes 

        worker_job_gather_index = step_state.worker_selected_loc_idx[:,:,:,0:max_job_in_worker_size,None].repeat(
            1,1,1,1,2 # embedding_dim
        ) # shape: [ batch, (pomo, worker, max_jobs), embedding ]


        current_job_gathering_index = self.reset_state.job_loc_idx[
            :,:,step_state.current_job_idx:step_state.current_job_idx+1
            ][:,:,None,None,:].repeat(1,1,1,1,2) # 2 is addr X.Y size

        pick_gathering_index = self.reset_state.job_loc_idx[
            :,:,step_state.current_job_idx:step_state.current_job_idx+1
            ].repeat(1,1,2) # 2 is addr X.Y size



        current_job_gather_index = step_state.worker_selected_loc_idx[:,:,:,0:max_job_in_worker_size,None].repeat(
            1,1,1,1, 2 # embedding_dim
        ) # shape: [ batch, (pomo, worker, max_jobs), embedding ]
        ...
        return encoded_workers_mean, encoded_jobs_mean
