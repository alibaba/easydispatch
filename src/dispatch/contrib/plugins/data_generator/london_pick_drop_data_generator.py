...
def generate_all_workers(worker_count):
    list_to_insert = []
    for w_index in range(worker_count):
        loc_index = random.randint(0, len(job_addr) - 1)
        gps = job_addr[loc_index]
        gps["code"] = "W{}_{}".format(w_index, loc_index)
        gps["geo_latitude"] = gps["geo_latitude"] + (random.random() - 0.5) * 0.2
        gps["geo_longitude"] = gps["geo_longitude"] + (random.random() - 0.5) * 0.2
...
def generate_one_day_orders(current_day, worker_list, nbr_jobs, job_start_index, auto_planning_flag):
    ...
    for job_i in range(nbr_jobs):
        # 首先是-取-的任务

        loc_index = random.randint(0, len(job_addr) - 1)
        gps = job_addr[loc_index]
        gps["code"] = "j_{}_pick_{}".format(loc_index, job_i)
        gps["geo_latitude"] = gps["geo_latitude"] + (random.random() - 0.5) * 0.1
        gps["geo_longitude"] = gps["geo_longitude"] + (random.random() - 0.5) * 0.1

        job_code = "{}_{}_pick".format(loc_index, job_i)
        myobj = {
            "code": job_code,
        ...
        list_to_insert.append(myobj)
        job_1_code = myobj["code"]

        # 送的任务

        loc_index = random.randint(0, len(job_addr) - 1)
        gps = job_addr[loc_index]
        gps["code"] = "j_{}_drop_{}".format(loc_index, job_i)
        gps["geo_latitude"] = gps["geo_latitude"] + (random.random() - 0.5) * 0.1
        gps["geo_longitude"] = gps["geo_longitude"] + (random.random() - 0.5) * 0.1

        job_code = "{}_{}_drop".format(loc_index, job_i)
        # job_code = "{}-{}-drop-{}".format(
        ...
        myobj = {
            ...
            "flex_form_data": {
                ...
                "max_number_of_workers": 1,
                "priority": 1,
                "included_job_codes": [job_1_code, job_2_code]
            },
            "team": {
                "code": "london_t1",
                "name": "london_t1",
            ...
            "location": gps,
            "planning_status": JobPlanningStatus.UNPLANNED,
            "requested_duration_minutes": FIXED_JOB_DURATION * 2,
            "scheduled_duration_minutes": FIXED_JOB_DURATION * 2,
            "requested_start_datetime": datetime.strftime(
                current_day + timedelta(minutes=880), "%Y-%m-%dT%H:%M:%S"
            ),
            "scheduled_start_datetime": datetime.strftime(
...
def generate_all(opts):
    ...
    worker_list = generate_all_workers(worker_count=opts["generate_worker_count"])
    if opts["generate_worker_count"] > 0:
