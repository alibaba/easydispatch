...
def generate_all_workers():
    ...
    for loc_index, worker in worker_df.iterrows():
        ...
        myobj = {
            ...
            "location": {
                "code": gps["code"],
                "geo_latitude": gps["geo_latitude"] + (random.random() - 0.5) * 0.1,
                "geo_longitude": gps["geo_longitude"] + (random.random() - 0.5) * 0.1,
            },
            "flex_form_data": {
                ...
                "is_assistant": False,
                # UU跑腿 Specific
                "maxAcceptOrderCount": random.randint(6, 14),
            },
            # "weekly_working_minutes": "[ [0, 0], [480, 1140],[480, 1140],[480, 1140],[480, 1140],[480, 1140],  [0, 0]]",
            # 'level': 0,
            "tags": [],
...
def generate_one_day_orders(current_day, worker_list, nbr_jobs, job_start_index, auto_planning_flag):
    ...
    job_df = pd.read_csv(
        "{}/contrib/plugins/data_generator/sample_data/london_realtime_job_spec.csv".format(
            config.basedir),
        header=0,
        sep=";",
        encoding="utf_8",
    ...
    for _, job_schedule_type in job_df.iterrows():
        ...
        for job_i in range(job_schedule_type["Quantity"]):
            ...
            myobj = {
                ...
                "location": gps,
                "planning_status": JobPlanningStatus.UNPLANNED,
                "requested_duration_minutes": job_schedule_type["Duration"] * 2,
                "scheduled_duration_minutes": job_schedule_type["Duration"] * 2,
                "requested_start_datetime": datetime.strftime(
                    current_day + timedelta(minutes=880), "%Y-%m-%dT%H:%M:%S"
                ),
                "scheduled_start_datetime": datetime.strftime(
