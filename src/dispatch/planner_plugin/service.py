...
def get_by_plugin_id_and_serviceid(*, db_session, plugin_id: int,service_id: int ) -> Optional[ServicePlugin]:
    """
    Get a service_plugin by plugin contact id.
    """
    return db_session.query(ServicePlugin).filter(ServicePlugin.plugin_id == plugin_id).filter(ServicePlugin.service_id == service_id).first()
...
def create(*, db_session, service_plugin_in: ServicePluginCreate) -> ServicePlugin:
    ...
    if service is None :
        raise HTTPException(status_code=400, detail="service is None. Failed.")
    ...
    if plugin is None:
        raise HTTPException(status_code=400, detail=f"Failed to load plugin with slug {service_plugin_in.plugin.slug}.")

    if service_plugin_in.config:
        plugin.config.update(service_plugin_in.config)
        
    service_plugin = get_by_plugin_id_and_serviceid(db_session=db_session, plugin_id=plugin.id,service_id=service.id)
    if not service_plugin:
...
def switch_agent_plugin_for_service(
    ...
    the_planner_service = service_service.get(db_session=db_session, service_id=service_id)
    ...
    new_service_plugin = ServicePlugin(
        planning_plugin_type=service_plugin_type,
